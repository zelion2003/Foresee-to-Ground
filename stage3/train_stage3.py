...
DEFAULT_STAGE3_DATA = REPO_ROOT / "data" / "annotations" / "stage3_ft.json"
DEFAULT_STAGE3_OUTPUT = REPO_ROOT / "outputs" / "stage3"
DEFAULT_VIDEO_ROOTS = {
...
def parse_args():
    ...
    ap.add_argument("--data_path", default=str(DEFAULT_STAGE3_DATA), help="Path to the Stage-3 annotation JSON file.")
    ap.add_argument("--video_roots", type=str, default="", help='JSON mapping such as {"anet": "..."}')
    ...
    ap.add_argument("--output_dir", default=str(DEFAULT_STAGE3_OUTPUT), help="Directory used to store Stage-3 checkpoints.")
    ...
    ap.add_argument("--swanlab_project", default="stage3_vtg", help="SwanLab project name.")
...
def main():
    ...
    model_base = Qwen3VLForConditionalGeneration.from_pretrained(
        args.model_id, torch_dtype=torch.bfloat16, local_files_only=True
    ).to(device)
    try:
